...
const sheetData = async () => {
  const data = await fetch(
    "https://spreadsheets.google.com/feeds/list/1kDcLSk7Kh-1plq0QTw7C_QceiD-P6MRof9UJ9liVACI/4/public/values?alt=json"
  )
    .then((data) => data.json())
    .then((json) => {
      const formattedData = json.feed.entry.reduce((formattedData, item) => {
        formattedData.push({
          ...
          meat: item.gsx$stockistsofmeat.$t,
          vegetables: item.gsx$stockistsofvegetables.$t,
          bakerygoods: item.gsx$stockistsofbakerygoods.$t,
          pantrygoods: item.gsx$stockistsofpantrygoods.$t,
          ...
          same_day: item.gsx$samedaydeliveries.$t,
          next_day: item.gsx$nextdaydeliveries.$t,
          collection_points: item.gsx$collectionpoints.$t,
        });
...
export default function Grocery() {
  ...
  return (
    <div className="container">
      ...
      <Head>
        <title>List of South African Grocers that offer delivery</title>
      ...
      <Main>
        <Header>
          ...
          <h2>
            A growing list of {data ? data.length : ""} South African grocers (
            <button
              onClick={() => {
                ...
                if (confirming) {
                  ...
                  const newWindow = window.open(
                    "https://forms.gle/UuEkABZFP6RX5UFy9",
                    id
                  );
                  newWindow.focus();
                }
        ...
        {status === "loading" ? (
          ...
          <>
            <Table columns={columns} data={data} tableFor="alcohol"></Table>
          </>
        )}
      </Main>
    </div>
